...
def image_message_log_fields(message: str, limit: int = IMAGE_MESSAGE_PREVIEW_CHARS) -> dict[str, Any]:
    text = str(message or "").strip()
    if not text:
        return {"message_len": 0, "message_preview": "", "message_truncated": False}
    preview = re.sub(r"\s+", " ", text)[:limit]
    return {
        "message_len": len(text),
        "message_preview": preview,
        "message_truncated": len(text) > limit,
    }
...
def attach_image_error_context(target: Exception, source: Exception) -> Exception:
...
def stream_image_outputs(
    ...
    message_log_fields = image_message_log_fields(message)
    logger.info({
    ...
    if message and not file_ids and not sediment_ids and (last.get("blocked") or terminal_message):
        ...
        detailed_error = _get_detailed_error_from_tasks(backend, conversation_id) if last.get("blocked") else ""
    ...
    try:
        ...
        if isinstance(exc, ImagePollTimeoutError) and message and not file_ids and not sediment_ids:
            exc.conversation_id = conversation_id
            exc.upstream_message_len = len(message)
            exc.upstream_message_preview = message_log_fields["message_preview"]
            exc.upstream_message_truncated = message_log_fields["message_truncated"]
            exc.tool_invoked = last.get("tool_invoked")
            exc.turn_use_case = last.get("turn_use_case")
            exc.blocked = last.get("blocked")
            exc.terminal_message = terminal_message
...
def _iter_single_image_outputs(
        request: ConversationRequest,
        index: int,
        total: int,
) -> Iterator[ImageOutput]:
    ...
    while True:
        try:
            ...
            if last_retry_exception is not None:
                image_error = ImageGenerationError(
                    image_stream_error_message(str(last_retry_exception)),
                    account_email=getattr(last_retry_exception, "account_email", account_email),
                    conversation_id=getattr(last_retry_exception, "conversation_id", ""),
                )
                attach_image_error_context(image_error, last_retry_exception)
                raise image_error from exc
            if last_retry_error:
                raise ImageGenerationError(image_stream_error_message(last_retry_error), account_email=account_email) from exc
        ...
        try:
            ...
            for output in stream_fn(backend, request, index, total):
                ...
                last_output_conversation_id = output.conversation_id
                yield output
            if returned_message:
            ...
            if not returned_result:
                ...
                if emitted_for_token:
                    conv_id = last_output_conversation_id
                    raise ImageGenerationError(
            ...
            logger.warning({
                ...
                "error": str(exc),
                "conversation_id": getattr(exc, "conversation_id", ""),
                "upstream_message_len": getattr(exc, "upstream_message_len", 0),
                "upstream_message_preview": getattr(exc, "upstream_message_preview", ""),
                "upstream_message_truncated": getattr(exc, "upstream_message_truncated", False),
                "tool_invoked": getattr(exc, "tool_invoked", None),
                "turn_use_case": getattr(exc, "turn_use_case", ""),
                "blocked": getattr(exc, "blocked", None),
                "terminal_message": getattr(exc, "terminal_message", None),
                "index": index,
            })
            # 轮询超时：换账号重试
...
def _generate_single_image(
    ...
    return list(_iter_single_image_outputs(request, index, total))
...
def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    ...
    if request.n <= 1:
        # 单张图片，直接执行（无需线程池开销）
        yield from _iter_single_image_outputs(request, 1, 1)
        return
    ...
    if not config.image_parallel_generation:
        ...
        for index in range(1, request.n + 1):
            yield from _iter_single_image_outputs(request, index, request.n)
        return
    ...
    if not emitted:
        ...
        image_error = ImageGenerationError(
            ...
            account_email=getattr(last_exception, "account_email", "") if last_exception else "",
            conversation_id=getattr(last_exception, "conversation_id", "") if last_exception else "",
        )
        if last_exception:
            attach_image_error_context(image_error, last_exception)
        raise image_error
