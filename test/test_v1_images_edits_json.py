...
import api.image_inputs as image_inputs
...
class ImageEditsJsonApiTests(unittest.TestCase):
    ...
    def test_image_edit_rejects_oversized_multipart_file(self):
        with mock.patch.object(image_inputs, "MAX_IMAGE_REFERENCE_BYTES", 3):
            response = self.client.post(
                "/v1/images/edits",
                headers=AUTH_HEADERS,
                data={"prompt": "oversized multipart", "model": "gpt-image-2"},
                files=[("image", ("big.png", b"abcd", "image/png"))],
            )

        self.assertEqual(response.status_code, 400, response.text)
        self.assertIn("image file exceeds 50MB limit", response.text)
        self.assertFalse(self.calls)

    def test_image_edit_reports_remote_json_url_fetch_failure(self):
        fake_response = mock.Mock(status_code=404, headers={}, content=b"")
        with mock.patch.object(image_inputs.requests, "get", return_value=fake_response):
            response = self.client.post(
                "/v1/images/edits",
                headers=AUTH_HEADERS,
